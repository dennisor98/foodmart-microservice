...
import com.opensoft.foodmart.enums.JwtType;
...
public class JwtService {
	@Value("${JWT_EXPIRY_TIME:3600000000}")
	Long jwtExpiryTime;

	@Value("${REFRESH_JWT_EXPIRY_TIME:60000}")
	...
	public String generateToken(User user) {
		try {
			...
			claims.put("token_type", JwtType.USER_ACCESS_TOKEN.getToken());
	...
	public String generateVendorAccessToken(User user) {
		try {
			Map<String, Object> claims = new HashMap<>();
			claims.put("id", user.getId());
			claims.put("token_type", JwtType.VENDOR_ACCESS_TOKEN.getToken());
			claims.put("firstName",user.getFirstName());
			return Jwts.builder().setClaims(claims).setSubject(user.getId().toString()).setIssuedAt(new Date())
					.setExpiration(new Date(System.currentTimeMillis() + jwtExpiryTime))// 10 days validity
					.setId(UUID.randomUUID().toString())
 ...
 public String generateVendorRefreshToken(User user) {
	 try {
			Map<String, Object> claims = new HashMap<>();
			claims.put("id", user.getId());
			claims.put("token_type", JwtType.VENDOR_REFRESH_TOKEN.getToken());
			claims.put("firstName",user.getFirstName());
			return Jwts.builder().setClaims(claims).setSubject(user.getId().toString()).setIssuedAt(new Date())
					.setExpiration(new Date(System.currentTimeMillis() + jwtExpiryTime))// 10 days validity
					.setId(UUID.randomUUID().toString())
	...
	public String generateRefreshToken(User user) {
		try {
			Map<String, Object> claims = new HashMap<>();
			claims.put("id", user.getId());
			claims.put("token_type", JwtType.USER_REFRESH_TOKEN);
			claims.put("firstName", user.getFirstName());
			return Jwts.builder().setClaims(claims).setSubject(user.getId().toString()).setIssuedAt(new Date())
